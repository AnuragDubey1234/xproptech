...
export function NewsCard({ article, size = 'default', compactHeadline = false }: NewsCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group bg-white/80 backdrop-blur-sm rounded-2xl border border-red-100 hover:border-fire-red shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col h-full"
    >
      <Link href={`/news/${article.slug}`} className="block flex-grow flex flex-col h-full">
        <div className={`relative w-full overflow-hidden bg-neutral-200 ${size === 'large' ? 'aspect-[16/10]' : 'aspect-video'} flex-shrink-0`}>
          <Image
            src={imgSrc}
            alt={article.title}
            fill
        ...
        <div className="p-4 md:p-5 flex flex-col flex-grow">
          ...
          <h3 className={`gibson-bold text-gray-dark group-hover:text-fire-red transition-colors ${compactHeadline
